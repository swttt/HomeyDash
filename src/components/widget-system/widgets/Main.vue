...
<div class="powermeter" v-if="!loading">
  <q-list no-border>
    <q-item class="text-teal" v-if="widget.settings.room">{{device.zone.name}}</q-item>
    <q-item class="text-white">{{device.name}}<i class="fa fa-bolt fa-2x icon" aria-hidden="true" v-if="widget.settings.icon"></i></q-item>
  ...
  methods: {
    async getPowermeter () {
      this.device = await this.$homey.devices.getDevice({
        id: this.widget.settings.pusage
      })
